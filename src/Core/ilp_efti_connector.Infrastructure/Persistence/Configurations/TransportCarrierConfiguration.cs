...
public class TransportCarrierConfiguration : IEntityTypeConfiguration<TransportCarrier>
{
    public void Configure(EntityTypeBuilder<TransportCarrier> builder)
    {
        builder.ToTable("transport_carriers");
        ...
        builder.Property(c => c.TractorPlate)
            .HasColumnName("tractor_plate").HasMaxLength(20).IsRequired();

        builder.Property(c => c.EquipmentCategory)
            .HasColumnName("equipment_category").HasConversion<string>().HasMaxLength(20);

        builder.HasOne(c => c.TransportOperation)
            .WithMany(t => t.Carriers)
